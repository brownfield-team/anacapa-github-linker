class RepoCollaboratorsJob < CourseJob
  ...
  def perform(course_id)
    ActiveRecord::Base.connection_pool.with_connection do
      ...
      course_repos.each do |repo|
        ...
        total_repos_matched += users_matched != 0 ? 1 : 0  # Repo is only counted in summary if it was matched to >= 1 user
      end
      summary = "#{total_users_matched} collaborators found for #{total_repos_matched} repositories."
      create_completed_job_record(course_id, summary) # Replace this when rebasing other PR changes
    end
  end
  ...
  def users_for_repo(repo_record, students)
    ...
    repo_collaborators_req = github_machine_user.collaborators(repo_record.name, affiliation: "direct")
    ...
    students.each do |student|
      unless student.user.nil? or student.user.username.nil?
        ...
        if is_substring_matched || is_api_matched
          repo_contributor_record = RepoContributor.create(user: s_user, github_repo: repo_record,
