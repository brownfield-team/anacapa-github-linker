module Slack
  class CommandsController < ApplicationController
    ...
    def whois
      ...
      pluck_user_id_regex = /<@([^|]*)[|>]/
      user_id_matches = command_text.match(pluck_user_id_regex)
      if user_id_matches.empty?
        render json: { :text => "You must tag a Slack user."}
      else
        command_output = ""
        @students = workspace.course.roster_students
        user_id_matches.each do |match, index|
          ...
          if matched_student.present?
            ...
            github_str = github_id ? "[#{github_id}](https://github.com/#{github_id})" : "N/A"
            # Markdown linking to student show page
            student_name_str = "[#{matched_student.full_name}](#{course_roster_student_url(workspace.course.id, matched_student.id)})"
